...
class UpscaleProcessor(ParallelProcessor):
    ...
    def _get_parallel_kwargs(self, frame: ProcessedFrame) -> Dict[str, Any]:
        ...
        """
        return {"scale_factor": self.scale_factor, "interpolation": self.interpolation}

    @staticmethod
    def _process_frame_parallel(
        frame: ProcessedFrame, scale_factor: float, interpolation: int
    ) -> ProcessedFrame:
        ...
        result_data = cv2.resize(
            frame.data, (new_width, new_height), interpolation=interpolation
        )
    ...
    def update_processor_info(self, processor_info: ProcessorInfo) -> ProcessorInfo:
        return processor_info.set_frame_width_scale(self.scale_factor).set_frame_height_scale(self.scale_factor)
